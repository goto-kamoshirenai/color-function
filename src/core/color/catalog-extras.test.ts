...
describe("設計支援（ナッジ/CVDセーフ/補完/変換/並べ替え/ロール）", () => {
  ...
  it("rolesByOrder: FG/BG 指定と並び順でロールを決定する", () => {
    const roles = rolesByOrder(4, { fgIndex: 0, bgIndex: 3, accentIndex: 1 });
    const byRole = Object.fromEntries(roles.map((r) => [r.role, r.index]));
    expect(byRole.text).toBe(0);
    expect(byRole.background).toBe(3);
    expect(byRole.accent).toBe(1);
    // 残り（index 2）が primary
    expect(byRole.primary).toBe(2);
  });

  it("rolesByOrder: 無効な index は既定（bg=末尾・fg=先頭）に補正する", () => {
    const roles = rolesByOrder(3, { fgIndex: -1, bgIndex: 99 });
    const byRole = Object.fromEntries(roles.map((r) => [r.role, r.index]));
    expect(byRole.text).toBe(0);
    expect(byRole.background).toBe(2);
    expect(byRole.primary).toBe(1);
  });

  it("rolesByOrder: アクセントが BG と重なる場合はスキップして残りから補う", () => {
    const roles = rolesByOrder(4, { fgIndex: 0, bgIndex: 3, accentIndex: 3 });
    const byRole = Object.fromEntries(roles.map((r) => [r.role, r.index]));
    expect(byRole.accent).toBeUndefined();
    expect(byRole.background).toBe(3);
    expect(byRole.primary).toBe(1);
    expect(byRole.neutral).toBe(2);
  });
});
