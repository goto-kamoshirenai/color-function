...
> **実装時補正（2026-06-11）**: `--text-3` はモック値（light `#A0A0A6` / dark `#5E5E66`）が
> 自身の地に対して WCAG AA 不適合（2.6:1 等）だったため、**light `#69696F` / dark `#8E8E97`** に補正した
> （最悪地でも 4.5:1 以上）。axe による E2E 検査が検出。「自身がアクセシブル」（[02 §4]）がモック値に優先する。
