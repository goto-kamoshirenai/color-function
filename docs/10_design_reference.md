...
| 項目 | モック実装 | ドキュメント仕様 | 判断 |
|------|-----------|----------------|------|
| 色差ΔE | **ΔE76**（Lab ユークリッド距離） | [07 §7](./07_card_calculation_specs.md) は **CIEDE2000** | 要判断: 精度(2000) vs モック踏襲(76) |
| 色覚シミュレーション | 固定3×3行列・**sRGB空間**で適用 | [06 §5](./06_static_assets_schema.md) は **Machado2009・線形RGB** | 要判断: 精度 vs 簡易 |
| 調和スキーム | **HSV色相回転** | [06 §4.1](./06_static_assets_schema.md) は **OKLCH色相**推奨 | 要判断: 知覚均等(OKLCH) vs モック(HSV) |
| sRGB→XYZ係数 | 丸め（0.4124 等） | [07 §7.1] は高精度（0.4124564 等） | 実装は高精度に寄せる（無害） |
| 静的アセット | コード内ハードコード（色名18件等） | [06](./06_static_assets_schema.md) は外部JSON | 実装はJSON化（モックは内蔵） |
| 状態キー | index 方式 | [05](./05_data_model_and_card_contract.md) は id 方式 | 実装は id 方式（安定性） |

> 方針案: **UI/挙動/トークン/カード構成はモックに完全準拠**し、**計算の中身（ΔE/CVD/調和）は精度の高いドキュメント仕様に差し替える**（ユーザーへ確認）。
> 数値ラベルの閾値（ΔE<2 ほぼ同一 等）は採用する計算式に合わせて再較正する。
...
1. **§6の3つの計算差分**（ΔE / CVD / 調和の色空間）をどちらに寄せるか確定。
