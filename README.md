...
書籍が出る場所は 4 か所で、いずれにも PR 表記を添えている:

| 場所                          | 内容                                     |
| ----------------------------- | ---------------------------------------- |
| 図書館（`/library`）          | 蔵書一覧と、指標から書籍を引く索引       |
| 書籍の詳細（`/library/[id]`） | 本文・扱う指標・購入導線                 |
| カードの本マーク（参考資料）  | その指標を扱う書籍を最大 2 冊            |
| カード末尾の「次の一手」      | 基準未達など、結果に連動したときだけ表示 |
...
結果連動の導線（どの指標のどの状態でどの書籍を出すか）は
`src/features/cards/BookNudge.tsx` の `NUDGE` に定義し、
条件は各カードから `when` で渡す。
